...
class MixedMediaHandler(BaseMediaHandler):
    def supports(self, message_or_group):
        if not isinstance(message_or_group, list):
            return False  # Обрабатываем только группы

        # Проверяем, что есть хотя бы два разных типа медиа
        has_photo = any(
        ...
        # Поддерживаем, если есть хотя бы два разных типа
    ...
    async def handle(self, messages, target_topic_id):
        ...
        # Отправляем как отдельные файлы (Telegram не поддерживает смешанные альбомы)
        sent_messages = []
